...
public:
	...
	void sleeping_time_cal(string& s) {
		...
		if (int(s[12] - '0') > 0) {
			...
			m += 6; // 23시xx분이 시작시간이면 수면 결과 6분 bias 생기는 버그 땜빵
			while (m >= 60) {
				m -= 60;
				h++;
			}
	...
	void table_cal() {
		...
		for (auto x : for_table_data) { // 시작시간의 h와 m, 그 시간
			...
			int row_bias = 5; // 7시부터 시작이던걸 6시부터 시작으로 하니까 위 숫자를 침범. 그걸 조절하는 bias
			if (row <= 12) {
				row -= row_bias;
			}
			else if (row <= 17) {
	...
	void todaypercent_cal() { // 하루 여덟시간 집중은 100%
		int resttime = 30;
		todaypercent[0] = (O_time[0] * 100) / (60 * 8); // 8시간
		/*for (int i = 1; i < 4; ++i) {
...
int main()
{
	...
	int date = stoi(filename); // file 이름은 20xxxx
	int whatweek;
	string yoil_st;
	if (date / 10000 == 20) {
	...
	YEAR = filename_out.substr(0, 2);
	MONTH = filename_out.substr(2, 2);
	DAY = filename_out.substr(4, 2);
	cout << toDBTEXT << endl;
	string query_str = "INSERT INTO pTEXT (YEAR, MONTH, DAY, TEXT) VALUES (" + YEAR + " , " + MONTH + " , " + DAY + " , '" + toDBTEXT + "')";
	char* query1 = new char[query_str.length()];
	for (int i = 0; i < query_str.length(); ++i) {
		query1[i] = query_str[i];
	}
	query1[query_str.length()] = ')';
	const char* query_out1 = query1;*/
	...
	string query_string = "INSERT INTO result (YEAR, MONTH, DAY, PERCENT,SLEEPTIME,EXERCISE,WEEKNUM,YOIL) VALUES (" + YEAR + " , " + MONTH + " , " + DAY + " , '" + (go.ret_todaypercent()) + "' , '" + go.ret_sleeptime() + "', '" + today_exercise + "' , " + to_string(whatweek) + ",'" + yoil_st + "')";
